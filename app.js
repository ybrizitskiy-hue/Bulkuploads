...
const DISABLE_BOOSTS_VALUE = "BET_BOOST;acca_boost;TRENDING_PRECANNED_ACCA_BOOST";
const HEADERS = [
  "playerId",
  "brandId",
  "playerAlias",
...
const els = {
  ...
  priorityPillDisplay: document.getElementById("priorityPillDisplay"),
  inPlayBetDelay: document.getElementById("inPlayBetDelay"),
  disableCO: document.getElementById("disableCO"),
  disableBoosts: document.getElementById("disableBoosts"),
  ...
  disableBoostPill: document.getElementById("disableBoostPill"),
...
function getSettings() {
  return {
    ...
    disableBoosts: els.disableBoosts.checked,
...
function buildRows(accounts, settings) {
  return accounts.map((playerId) => ({
    ...
    Resolved: "",
    disableBoost: settings.disableBoosts ? DISABLE_BOOSTS_VALUE : ""
  }));
}
...
function makeFilename(category, index, total) {
  const safeCategory = category.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "upload";
  if (total > 1) {
...
function createZip(files) {
  ...
  files.forEach((file) => {
    ...
    const localHeader = concatArrays([
      ...
      u32(dataBytes.length), u32(dataBytes.length), u16(nameBytes.length), u16(0), nameBytes
    ]);
    localParts.push(localHeader, dataBytes);

    const centralHeader = concatArrays([
      u32(0x02014b50), u16(20), u16(20), u16(0), u16(0), u16(dosTime), u16(dosDate), u32(crc),
      u32(dataBytes.length), u32(dataBytes.length), u16(nameBytes.length), u16(0), u16(0), u16(0),
...
function updateUi() {
  ...
  els.statusText.textContent = !hasAccounts
    ? "Paste player IDs to start.\nPlayer note is mandatory and must be at least 5 symbols."
    : !noteIsValid
      ...
      : isZip
        ...
        : "Ready to generate one CSV file.\nMaximum per single file is 98 players.";

  els.disableCOPill.textContent = settings.disableCO ? "YES / TRUE" : "NO / empty";
  els.disableBoostPill.textContent = settings.disableBoosts ? DISABLE_BOOSTS_VALUE : "NO / empty";
  ...
  const priorityLabel = priorityLabels[settings.playerNotePriority] || "Neutral / 2";
  if (els.priorityPill) els.priorityPill.textContent = priorityLabel;
  if (els.priorityPillDisplay) els.priorityPillDisplay.textContent = priorityLabel;
...
function handleDownload() {
  ...
  const safeCategory = settings.category.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "upload";
...
function resetForm() {
  ...
  els.disableBoosts.checked = false;
  updateUi();
}
...
function renderBrandResults() {
  ...
  if (!matches.length) {
    ...
    empty.textContent = "No match.\nPress Enter or click away to use this as a custom brand.";
...
let cutterState = { fileName: "", header: "", rows: [] };
...
function baseName(fileName) {
  return (fileName || "split-file")
    .replace(/\.[^.]+$/, "")
    .replace(/[^a-zA-Z0-9_-]+/g, "-")
    .replace(/^-+|-+$/g, "") || "split-file";
}
...
function updateCutterUi() {
  ...
  els.cutterStatusTitle.textContent = rows.length
    ? `${rows.length} data rows / ${fileCount} output file${fileCount === 1 ? "" : "s"}`
    : "Waiting for CSV";
  els.cutterStatusText.textContent = rows.length
    ? `Every split file will include the original header row.\nMaximum ${MAX_PLAYERS_PER_FILE} data rows per CSV.`
...
function loadCutterFile(file) {
  ...
  reader.onload = () => {
    try {
      ...
      cutterState = { fileName: file.name, header: records[0], rows: records.slice(1).filter((row) => row.trim() !== "") };
      showCutterError("");
      updateCutterUi();
    } catch (error) {
      clearCutter();
      showCutterError("Could not read this CSV.\nPlease check the file and try again.");
    }
  };
  reader.onerror = () => {
    clearCutter();
...
let betnotesState = { fileName: "", rows: [] };
...
function updateBetnotesUi() {
  ...
  els.betnotesStatusTitle.textContent = rows.length
    ? `${rows.length} betIds / ${fileCount} output file${fileCount === 1 ? "" : "s"}`
    : "Waiting for report";
  ...
  reader.onload = () => {
    try {
      ...
      if (isCsv) {
        ...
        if (!window.XLSX) throw new Error("XLSX parser is still loading.\nPlease refresh the page and try again.");
      ...
      clearBetnotes();
      showBetnotesError(error.message || "Could not read this file.\nPlease check the template and try again.");
    }
  };
  reader.onerror = () => {
    clearBetnotes();
    showBetnotesError("Could not read this file.\nPlease check it and try again.");
  };
...
function setupDropZone(dropElement, fileInput, loader) {
  dropElement.addEventListener("click", () => fileInput.click());
  ...
  dropElement.addEventListener("drop", (event) => loader(event.dataTransfer.files[0]));
}
...
els.categorySearch.addEventListener("change", () => chooseCategory(els.categorySearch.value));
...
[els.accounts, els.playerNote, els.playerNotePriority, els.inPlayBetDelay, els.disableCO, els.disableBoosts, els.betFactor].forEach((el) => {
...
setupDropZone(els.fileDrop, els.cutterFile, loadCutterFile);
setupDropZone(els.betnotesDrop, els.betnotesFile, loadBetnotesFile);
